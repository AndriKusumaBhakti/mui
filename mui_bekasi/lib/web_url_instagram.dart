...
class WebUrlIntegram extends KFDrawerContent {
  WebUrlIntegram({Key key, this.url});
...
class _MainPageState extends State<WebUrlIntegram> {
  ...
  void service() async{
    Config.log('IG : '+(await Prefs.getITG).toString());
    url = (await Prefs.getITG) != null && (await Prefs.getITG).isNotEmpty ? await Prefs.getITG:widget.url;
    setState(() {});
  }
          ...
          Container(
            ...
            child: Row(
              children: <Widget>[
                ...
                Text("Instagram", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white), textAlign: TextAlign.center,),
                Spacer(),
                InkWell(
                  child: Container(
                    height: 40,
