    ...
    <section class="section dashboard">
     ...
     <div class="col-12">
              <div class="card recent-sales overflow-auto">
                <div class="card-body">
                  <h5 class="card-title"><span>Berita</span></h5>
                  <form action="tambah_data.php" method="post">
                    <input type="hidden" name="add" id="add" value="<?php echo 'berita';?>"/>
                    <button type="submit" class="btn btn-primary">Tambah</button>
                  </form>
                  <br>
                  <table id="user-list" class="table table-borderless datatable">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                    <?php 
                      $brg=mysqli_query($conn, "select a.id,a.judul, a.berita, b.nama, a.image_src,
                       a.tanggal from berita a join master_data b on a.kategori_code = b.kode order by a.id DESC");
                      $no=1;
                      while($b=mysqli_fetch_array($brg)){
                        ...
                        <tr>
                          ...
                          <td>
                            <?php 
                              if(strlen($b['berita'])>300){
                                echo substr($b['berita'], 0, 300);
                              } else {
                                echo $b['berita'];
                              } 
                            ?>
                            ...
                            <form action="view_data.php" method="post">
                                ...
                                <input type="hidden" name="show" id="show" value="<?php echo 'berita';?>"/>
                                <button type="submit"  title="Show detail" class="bi bi-eye-fill"> Show detail</button>
                            </form>
                          </td>
                          <td><?php echo $b['nama'] ?></td>
                          <td><?php echo $b['tanggal'] ?></td>
                          <td><img style="max-height:100px;background-color:blue;" class="rounded mx-auto" src="https://muitvkotabekasi.com/img/<?php echo $b['image_src']?>" class="card-img pt-4" alt="..."></td>
                          <td>
                            <form action="edit_data.php" method="post">
                                <input type="hidden" name="id" id="id" value="<?php echo $b['id'];?>"/>
                                <input type="hidden" name="update" id="update" value="<?php echo 'berita';?>"/>
                                <button type="submit" class="bi bi-pencil-square"></button>
                            </form>
                            <form action="hapus_data.php" method="get">
                                <input type="hidden" name="id" id="id" value="<?php echo $b['id'];?>"/>
                                <input type="hidden" name="from" id="from" value="<?php echo 'berita';?>"/>
